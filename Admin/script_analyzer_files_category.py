...
def get_category_counts(roots):
    ...
    # 개수 많은 순(내림차순), 개수가 같으면 (1차, 2차) 가나다순으로 안정적인 정렬
...
class CategoryWidget:
    def __init__(self, roots):
        ...
        style.configure('Custom.Treeview', background='#222222', foreground='#dddddd',
                         fieldbackground='#222222', font=('맑은 고딕', 10), rowheight=22, borderwidth=0)
        style.configure('Custom.Treeview.Heading', background='#333333', foreground='white',
                         font=('맑은 고딕', 9, 'bold'))
        style.map('Custom.Treeview', background=[('selected', '#3a5a8a')])

        self.tree = ttk.Treeview(body, columns=('cat2', 'count'), show='tree headings',
                                  style='Custom.Treeview')
        self.tree.heading('#0', text='1차분류')
        self.tree.heading('cat2', text='2차분류')
        self.tree.heading('count', text='개수')
        self.tree.column('#0', anchor='w', width=90, stretch=True)
        self.tree.column('cat2', anchor='w', width=110, stretch=True)
        self.tree.column('count', anchor='e', width=40, stretch=False)
    ...
    # --- 행 더블클릭: Everything 실행 ---
    def on_row_double_click(self, event):
        row_id = self.tree.identify_row(event.y)
        if not row_id:
            return
        cat1 = self.tree.item(row_id, 'text')
        cat2 = self.tree.item(row_id, 'values')[0]
        self.open_in_everything(f'{cat1}_{cat2}')
    ...
    def _render(self, items, errors):
        ...
        if not self.tree.winfo_ismapped():
            self.tree.pack(side='left', fill='both', expand=True)
        for (cat1, cat2), count in items:
            self.tree.insert('', 'end', text=cat1, values=(cat2, count))
